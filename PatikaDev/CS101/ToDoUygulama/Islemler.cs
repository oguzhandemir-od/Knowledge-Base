...
namespace ToDoUygulama
{
    internal class Islemler
    {
        ...
        public static void KartGuncelle()
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    Console.WriteLine($"'{guncellenecekKart[0].Baslik}' kartının güncellenmesi yeni bilgilerle gerçekleştirilecektir. Onaylıyor musunuz? (Y/N)");
                    string onay = Console.ReadLine();
                    if (onay.ToUpper() == "Y")
                    {
        ...
        public static void BoardListele()
        {
            ...
            Console.WriteLine("*******************");
            ListeYaz(line.Todoline);
            
            Console.WriteLine();
            Console.WriteLine("IN PROGRESS LINE");
            Console.WriteLine("*******************");
            ListeYaz(line.Inprogressline);

            Console.WriteLine();
            Console.WriteLine("DONE LINE");
            Console.WriteLine("*******************");
            ListeYaz(line.Doneline);
        }
        #endregion
        ...
        public static List<Kart> KartAra(string baslik)
        {
            ...
            while (true)
            {
                AramaListe(line.Todoline, baslik, aramaSonuclari, kontrol);
                AramaListe(line.Inprogressline, baslik, aramaSonuclari, kontrol);
                AramaListe(line.Doneline, baslik, aramaSonuclari, kontrol);
        ...
        public static void AramaListe(List<Kart> l,string baslik,List<Kart> aramaSonuclari,bool kontrol)
        {
            foreach (var b in l)
            {
        ...
        public static void ListeleriSirala()
        {
            Line line=Line.Basla();
            line.Todoline.Sort((s1, s2) => s1.Baslik.CompareTo(s2.Baslik));
            line.Inprogressline.Sort((s1, s2) => s1.Baslik.CompareTo(s2.Baslik));
            line.Doneline.Sort((s1, s2) => s1.Baslik.CompareTo(s2.Baslik));
        }
        public static void ListeYaz(List<Kart> l)
        {
            if (l.Count == 0)
